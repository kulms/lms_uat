...
$action = $strSystem_LabNewsAdd ;

if ($id != "" && $id != none)
					{
						$news=mysql_query("SELECT  nc.category,nc.subject,nc.title,nc.id,nc.picture,nc.htmlfile, nc.expired_date, nc.news_area FROM news_system nc WHERE  nc.id=$id and users=".$person["id"]);
...
<script language="javascript" src="modules/system/include/cal.js"></script>
<script language="javascript" src="modules/system/include/cal_conf.js"></script>
...
<table width="80%" border="0" cellpadding="0" cellspacing="1" class="tdborder3">
                      <form action="?m=system&a=news_save"  method="post" enctype="multipart/form-data" name="myForm" id="myForm">
                       <input name="user_id" type="hidden" id="user_id" value="<?=$person["id"]?>">
...
fnInit(idContent);
                            ...
                            <tr> 
                                <td height="22" align="right" class="tdbackground"><b><?=$strSystem_LabNewsViewperson?></b>&nbsp;</td>
                                <td height="22" class="tdbackground1">
								<select name="category" style="font-size:9px ">
                <option value="0"><?=$strSystem_LabNewsSelectUser;?></option>
                <option value="1" <?php if($cate == "1") { echo "selected";}?>><?php echo $strSystem_LabNewsUserAdmin;?></option>
                <option value="2" <?php if($cate == "2") { echo "selected";}?>><?php echo $strSystem_LabNewsUserTeacher;?></option>
                <option value="3" <?php if($cate == "3") { echo "selected";}?>><?php echo $strSystem_LabNewsUserStudent;?></option>
                <option value="4" <?php if($cate == "4") { echo "selected";}?>><?php echo $strSystem_LabNewsUserShowall;?></option>
        ...
        <a href="javascript:showCal('Date1')"><img  align="absmiddle" src="images/date1.gif" style="cursor:pointer;cursor:hand" title="Click to select date"  onMouseOver="window.status='Click to select date';return true" onmouseout="window.status='';return true"  width="16" height="16" border="0"></a></td>
                              </tr>
                              <tr> 
                                <td height="22" align="right" class="tdbackground">&nbsp;</td>
         ...
         <?= $strSystem_LabNewsAnnounceSystem;?><br>
...
if(confirm('Are you sure to return to list?')) {
	document.location.href='?m=system&a=news&page=<?=$page;?>';
}
						" value="<?=$strCancel;?>"> </td>
                                    </tr>
                                  </table></td>
