...
//			$sql = "SELECT * FROM users WHERE active=".$active.$where." ORDER BY $order;";
	...
	function ShowTableAll($result,$user,$uistyle,$tab,$page,$stub) {
		...
		echo "</td>";
		echo "<th width=\"150\">";
		echo "<a href=\"?m=users&a=index&orderby=login&tab=$tab&page=$page&stub=$stub\" class=\"hdr\">UserName</a>";
		echo "</th>";
		echo "<th>";
		echo "<a href=\"?m=users&a=index&orderby=firstname&tab=$tab&page=$page&stub=$stub\" class=\"hdr\">Name-Surname</a>";
		echo "</th>";
		echo "<th>";
		echo "<a href=\"?m=users&a=index&orderby=lastlogin&tab=$tab&page=$page&stub=$stub\" class=\"hdr\">Last Login</a>";
		echo "</th>";
		echo "</tr>";
	...
	/*	$NRow = $result->numRows();																	 
		...
		if($page>1 && $page<=$totalpage) {
				$prevpage = $page-1;
				echo "\t<a href='./index.php?m=users&page=$prevpage&orderby=$order&stub=$stub&tab=$tab' class=a11>[Prev]</a>\n";
		}
		...
		if($page!=$totalpage) {
				$nextpage = $page+1;
				echo "\t<a href='./index.php?m=users&page=$nextpage&orderby=$order&stub=$stub&tab=$tab' class=a11>[Next]</a>\n";
		}
		...
		for($i=1 ; $i<$page ; $i++) {
				echo "\t<a href='./index.php?m=users&page=$i&orderby=$order&stub=$stub&tab=$tab'>$i</a> \n";
		}
		echo "\t<font size=2 color=red><b>$page</b></font> \n";
		for($i=$page+1 ; $i<=$totalpage ; $i++) {
				echo "\t<a href='./index.php?m=users&page=$i&orderby=$order&stub=$stub&tab=$tab' class=a11>$i</a> \n";
		}
...
function import($Login,$Pass,$FName,$Cat){
	...
	VALUES ('".$Login."','".$password."','".$FName."','".$Cat."',0) ";
	$result = $db->query($sql);	   
