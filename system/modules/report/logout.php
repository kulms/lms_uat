...
<form name="allTime" method="post" action="?m=report&a=logout&filter='time'&user_type=<? echo $user_type?>" onSubmit="return checkdate(this)">
...
<tr><td colspan="2">
<b><? echo $user->_($strSystem_RMenuLogout);?></b>
</td></tr>
  <tr>
    <td><strong><? echo $user->_($strSystem_RHeader);?></strong> : <a href="?m=report&a=logout&user_type=<? echo $user_type?>"><? echo $user->_($strSystem_RMenuAll);?></a> | <? echo $user->_($strSystem_LabReportTime);?><input type="text" name="beTime" value="<? if($beTime!="") echo $beTime?>" class="text" size="10" onFocus="this.blur();"> 
...
<table width="90%"  border="0" cellspacing="1" cellpadding="0" align="center" class="tdborder1">
  <tr class="boxcolor">
    <td width="23%" align="center"><a  class="a13" href="Javascript:OrderBy('logout','<? echo $filter?>','<? echo $beTime ?>','<? echo $endTime ?>','<? echo $page?>','time','<? echo $courses ?>','<? echo $user_type?>');"><? echo $user->_($strSystem_ListReportTime);?></a></td>
    <td width="29%" align="center"><a  class="a13" href="Javascript:OrderBy('logout','<? echo $filter?>','<? echo $beTime ?>','<? echo $endTime ?>','<? echo $page?>','login','<? echo $courses ?>','<? echo $user_type?>');"><? echo $user->_($strSystem_ListReportName);?></a></td>
