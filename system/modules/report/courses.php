...
<form name="allTime" method="post" action="?m=report&a=courses&filter='time'<? echo $course?>" onSubmit="return checkdate(this)">
<input type="hidden" name="filter" value="time" >
<table width="90%"  border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  <tr><td colspan="2">
<b><? echo $user->_($strSystem_RMenuCourse);?></b>
</td></tr>
  <tr>
    <td height="28" colspan="2"><strong><? echo $user->_($strSystem_RHeader);?></strong> : <a href="?m=report&a=courses&user_type=<? echo $user_type?><? echo $course?>"><? echo $user->_($strSystem_RMenuAll);?></a> | 
	<a href="?m=report&a=courses&filter=create&user_type=<? echo $user_type?><? echo $course?>"><? echo $user->_($strSystem_CReportCreate);?></a> | 
	<a href="?m=report&a=courses&filter=update&user_type=<? echo $user_type?><? echo $course?>"><? echo $user->_($strSystem_CReportUpdate);?></a> | 
	<a href="?m=report&a=courses&filter=delete&user_type=<? echo $user_type?><? echo $course?>"><? echo $user->_($strSystem_CReportDelete);?></a> | 
	<a href="?m=report&a=courses&filter=apply&user_type=<? echo $user_type?><? echo $course?>"><? echo $user->_($strSystem_CReportApply);?></a> | 
	<a href="?m=report&a=courses&filter=drop&user_type=<? echo $user_type?><? echo $course?>"><? echo $user->_($strSystem_CReportDrop);?></a> </td>
  </tr>
  <tr>
...
<table width="90%"  border="0" cellspacing="1" cellpadding="0" align="center" class="tdborder1">
  <tr class="boxcolor">
    <td width="23%" align="center"><a  class="a13" href="Javascript:OrderBy('courses','<? echo $filter?>','<? echo $beTime ?>','<? echo $endTime ?>','<? echo $page?>','time','<? echo $courses ?>','<? echo $user_type?>');"><? echo $user->_($strSystem_ListReportTime);?></a></td>
    <td width="29%" align="center"><a  class="a13" href="Javascript:OrderBy('courses','<? echo $filter?>','<? echo $beTime ?>','<? echo $endTime ?>','<? echo $page?>','login','<? echo $courses ?>','<? echo $user_type?>');"><? echo $user->_($strSystem_ListReportName);?></a></td>
