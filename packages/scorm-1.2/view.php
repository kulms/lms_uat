...
define('AT_PACKAGE_URL_BASE', 'http://'.$_SERVER["HTTP_HOST"].'/'.$_SESSION['path'].'/files/sco/');

$me = 'scorm-1.2/view.php';
$im = 'images/';
...
$student_name = $q_row['surname'] . ', ' . $q_row['firstname'];
...
width="0" height="0" >
<param name="student_id"   value="<?php echo $person["id"]?>" />
<param name="student_name" value="<?php echo $person["login"]?>" />
<?php	
	$prefs['show_rte_communication'] = 1;
	if ($prefs['show_rte_communication'] == 1) {
...
function LMSInitialize (s) {
	...
	o.alt   = '<?php echo 'scorm_sco_is_running'?>';
	o.title = '<?php echo 'scorm_sco_is_running'?>';
