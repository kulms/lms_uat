...
class package_handler_scorm_1_2 {
	...
	function getManagerItemLinks ($id) {
		...
		while ($row = mysql_fetch_assoc($result)) {
			array_push ($rv,
				'<div class="scormitem">'
				. '<a href="scorm-1.2/view.php?org_id='
				. $row['org_id'].'"'
	...
	function getLearnerItemLinks ($id) {
		...
		while ($row = mysql_fetch_assoc($result)) {
			array_push ($rv, ''
				. '<div class="scormitem">'
				. '<a href="scorm-1.2/learner_view.php?org_id='
				. $row['org_id'].'"'
			        . ' title="ADL SCORM-1.2 Package"'
				. ' onclick="show(\'scorm_1_2_throb_' . $row['org_id'] . '\')"'
				. '>'
	...
	function getCMILinks ($id) {
		...
		while ($row = mysql_fetch_assoc($result)) {
			array_push ($rv,
				'<div class="scormitem">'
				. '<a href="scorm-1.2/cmi.php?org_id='
				. $row['org_id'].'"'
	...
	function getSettingsLinks ($id) {
		...
		while ($row = mysql_fetch_assoc($result)) {
			array_push ($rv,
				'<div class="scormitem">'
				. '<a href="scorm-1.2/settings.php?org_id='
				. $row['org_id'].'"'
			        . ' title="ADL SCORM-1.2 Package"'
				. '>'
				.  $row['title'] .
	...
	function getReportLinks ($id) {
		$result = $this->getOrgs ($id);
		$rv = Array();
		while ($row = mysql_fetch_assoc($result)) {
			array_push ($rv,
				'<div class="scormitem">'
				. '<a href="scorm-1.2/report.php?org_id='
				. $row['org_id'].'"'
			        . ' title="ADL SCORM-1.2 Package"'
				. '>'
				.  $row['title'] .
	...
	function deletePackages ($pids, $courses) {
		global $msg;
		global $db;
		...
		foreach ($pids as $id) {
			...
			$sql = "DELETE	FROM p_scorm_1_2_item
				WHERE	org_id = $id";
			...
			$sql = "DELETE	FROM p_scorm_1_2_org
				WHERE	org_id = $id";
