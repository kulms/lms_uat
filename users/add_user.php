...
<form ENCTYPE="multipart/form-data" action="add_user.php" method="post"   name="user" target="ws_main">        
    <table border=0 cellpadding="2" cellspacing="0" class="tborder">
      ...
      <tr> 
        ...
        <td align="left" class="main"> <select name="category" class="pn-text">
			...
			?>
            <option value="<? echo $row_ut["id"];?>"><? echo $row_ut["nametype"];?></option>
            <? }?>
          </select> </td>
      </tr>
      <?php
...
//	  			{ 		?>
      ...
      <tr> 
        ...
        <td align="left" class="main"><input type="checkbox" name="active" value="1" class="r-button"></td>
      </tr>
      <tr> 
    ...
    <input type="hidden" name="id" value="<? //echo $person["id"]; ?>">
    <input type="hidden" name="update" value="true">
    <input type="submit" value="<? echo $strAdd;?>">
...
<?
	   ...
	   switch($category)
		...
		$ins_user = "INSERT INTO users (active,login,password,firstname,surname,email,homepage,picture,admin,lastlogin,category,title,ucode,email2,dept_id,fac_id,major_id) 
								VALUES ($active, '".trim($login)."', '".md5(trim($password))."','".trim($firstname)."','".trim($surname)."','".trim($email)."','".trim($homepage)."',
													'".trim($piz_name)."',0,0,$category,'".trim($title)."','".trim($ucode)."','".trim($email2)."','".trim($dept)."','".trim($fac)."','".trim($major)."');";
		mysql_query($ins_user);
		$newuser = mysql_insert_id();
		//echo $newuser;
		$workdir=$realpath."/files/preference/".$newuser."/";
	 	$workdir2=$realpath."/files/preference/".$newuser."/"."original/";
		$ins_user_info = "INSERT INTO users_info (id,address,skill_interest,telephone,internal_phone,facimile,room,building,mobile_phone,icq,office_tel,office_address,p_address,p_telephone,p_mobile_phone,postal,picture_width,picture_height,firstname_eng,surname_eng,title_eng) 
											VALUES( ".$newuser.",'".trim($address)."','".trim($skill_interest)."','".trim($telephone)."', '".trim($internal_phone)."','','".trim($room)."','".trim($building)."','".trim($mobile_phone)."','".trim($icq)." ','".trim($office_tel)."','".trim($office_address)."','".trim($p_address)."',
																'".trim($p_telephone)."','".trim($p_mobile_phone)."','".trim($postal)."','','','".trim($firstname_eng)."', '".trim($surname_eng)."','".trim($title_eng)."' )";								 			
		mysql_query($ins_user_info); 		
			  ...
			  {    						
				...
				{     
                        if((!copy($piz, $dest))||(!copy($piz, $dest2))) 
						{
								...
								// here
								   mysql_query("UPDATE users SET picture='".$f_dbname."' WHERE id=".$newuser.";");								   
                        	 }
                }
