...
var bar = new menuBar();
bar.addMenu('t_index2.php',' หน้าหลัก ');
bar.addItem('','');
...
var  already_point= '#009933';
var point= '#C8FF91';
var char_afterp= '#ffffff';
var char_before= '#009933';
...
function writeBar() {
		for (var i=1;i <= this.mO.length; i++){ 
			if(this.mO[i-1].mlink =='')  document.write('<span id="navMenu'+i+'" class="bwhite"> '+this.mO[i-1].main+' <\/span>');
			else document.write(' <a href="'+this.mO[i-1].mlink+'"><span id="navMenu'+i+'" class="bwhite"> '+this.mO[i-1].main+' <\/span><\/a>');
		if(i != this.mO.length) document.write('<span  id="navMenu'+i+'" class="bwhite">|<\/span>');
...
function writeDrop() {
	for (var i=1;i <= this.mO.length; i++){  
		//if(this.mO[i-1].name.length >0){  alert(this.mO[i-1].name.length);// 
...
window.onscroll=sMenu;window.onload=iMenu;var onm = null;var ponm = null;var podm = null;var ndm = bar.mO.length;
