...
function hideCal() {
  if (isIE) {
    layers[calWin].innerHTML="";
  }
  else if (isN4) {
    layers[calWin].visibility="hide";
    layers[calWin].document.open();
    layers[calWin].document.close();
  }
  else if (isN6) {
...
*/  
    ...
    if (isIE) {
      layers[calWin].style.pixelTop=getTopIE(layers[currentCal.id],true);
      layers[calWin].style.pixelLeft=getLeftIE(layers[currentCal.id],true);
      layers[calWin].innerHTML=calendar;

      layers[calWin].style.clip = "rect(0px; " + layers[calWin].children[0].offsetWidth + "px; " + layers[calWin].children[0].offsetHeight + "px; 0px)";
    }
    else if (isN4) {
      layers[calWin].top=getTopN4(layers[currentCal.id]);
      layers[calWin].left=getLeftN4(layers[currentCal.id]);
      layers[calWin].document.open();
      layers[calWin].document.writeln(calendar);
      layers[calWin].document.close();
      layers[calWin].visibility="show";
    }
    else if (isN6) {
      var l=document.getElementById(currentCal.id);
      layer=document.getElementById(calWin);
