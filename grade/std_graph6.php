...
$arrColor = array($pink, $blue_sea, $red, $black);
...
for ($i=1; $i<=3; $i++) {
  ...
  if ($i==1) {
  	if ($num_aa!=0) {
  		ImageString($img,2,$mid_x-10,$mid_y, 'G : '. $num_aa, $black);
  	}
  }
  
  if ($i==2) {
  	if ($num_bb!=0) {
  		ImageString($img,2,$mid_x-10,$mid_y, 'P : '. $num_bb, $black);
  	}
  }
  if ($i==3) {
  	if ($num_cc!=0) {
  		ImageString($img,2,$mid_x-10,$mid_y, 'F : '. $num_cc, $black);
  	}
  }
}
