...
if($modules_type<0){
	...
	//************************** m o d u l e s *****************************************
mysql_query("INSERT INTO modules (name,modules_type,users,updated,created,active,courses) values('".str_replace("'","&#039;",$name)."',$modules_type,".$person["id"].",".time().",".time().",1,$courses);");
	$modules=mysql_insert_id();
	//quiz
	if($modules_type==5){
		mysql_query("INSERT INTO q_module_prefs (module_id) VALUES (".$modules.")");
	}
//***********insert modules_history***************
$action="Create";
...
if($folders==""){
	...
	if(mysql_result($mt,0,"url_setup")==""){
		...
		<? }else{?>
			<META HTTP-EQUIV="Refresh" CONTENT="0;URL=../quiz/start.php?a=setup&m=admin&modules=<? echo $modules;?>">
		<? } ?>
		<script language="javascript">
			function update(){
				top.ws_menu.location.reload();
