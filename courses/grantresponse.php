...
$checkadmin=mysql_query("SELECT id FROM wp WHERE courses=$courses AND users=".$person["id"]." AND admin=1;");
if(mysql_num_rows($checkadmin)!=0)
{
        $ok=1;
}else{
        $checkcreator=mysql_query("SELECT name FROM courses WHERE id=$courses AND users=".$person["id"].";");
        if(mysql_num_rows($checkcreator)!=0)
		{
                $ok=1;
        }
}
$getinfo=mysql_query("SELECT c.applyopen,u.firstname,u.surname,u.login,c.name,u.email 
					  FROM courses c, users u 
					  WHERE c.id=$courses AND u.id=$users;");
...
$coursename=$row["name"];
 ...
 if(($ok==1 || $person["admin"]==1) && $row["applyopen"]==0)
 {       // OK, $admin = creator of course AND applyopen=0                                                                                                                              // (if 1 then no mail should have been sent out
         $check_apply=mysql_query("SELECT id FROM apply_courses WHERE users=$users AND courses=$courses;");
                 if(mysql_num_rows($check_apply)!=0)
				 { 
				        //OK, user NOT already in course - still in apply_courses
					   if($rnd==1)
					   { 	// User granted access
							...
							//mysql_query("DELETE FROM wait_for_grant WHERE id=$waitid;");
							 mysql_query("DELETE FROM wait_for_grant WHERE users=$users and courses=$courses;");
							// mysql_query("DELETE FROM apply_courses WHERE id=".mysql_result($check_apply,0,"id").";"); 
							//Clear record in apply_courses
							 mysql_query("DELETE FROM apply_courses WHERE users=$users and courses=$courses;"); 
							 $mailbody=$fname.",";
							 $mailbody.="\nYou have been accepted to join".$coursename.".\n\n Further information can be found 
							 when you visit the course pages.\n\nWelcome to Course On Web , Faculty of Engineering , KU";
 ...
 <?						} // $rnd=0 -> no access
                                ...
                                //if(mysql_query("DELETE FROM apply_courses WHERE id=".mysql_result($check_apply,0,"id").";"))
								if(mysql_query("DELETE FROM apply_courses WHERE users=$users and courses=$courses;"))
								{ //Clear record in apply_courses
								 //mysql_query("DELETE FROM wait_for_grant WHERE id=$waitid;");
								//mysql_query("DELETE FROM wait_for_grant WHERE users=$users and courses=$courses;");
                                   $mailbody.="\nWe are sorry to inform you that you were not accepted to 
join ".$coursename.".\n\nYou are nevertheless welcome to apply for other courses in Course on Web at
...
?>                         <table cellpadding="3" cellspacing="3" align="center">
                                 ...
                                 <tr>
                                         <td width="15%">&nbsp;</td>
                                         <td class="main" align="left" valign="top" colspan="2">If the problem persists - please contact 
 <a href="mailto:courseadmin@<? echo $SERVER_NAME; ?>">courseadmin@<? echo $SERVER_NAME; ?></a>
</td>
                                 </tr>
                         </table>
<?
