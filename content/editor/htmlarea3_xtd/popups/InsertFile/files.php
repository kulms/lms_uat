<?
// $Id: files.php, v 1.01.5 2004/04/19 12:04:21 bpfeifer Exp $
/**
...
function changeDir(Path) {
	location.href = "files.php?datadir=<?=$datadir?>&dialogname=<?php echo $MY_NAME; ?>&path="+Path;
}

function goUp() {
	location.href = "files.php?datadir=<?=$datadir?>&dialogname=<?php echo $MY_NAME.'&path='.$MY_UP_PATH; ?>";
}

function newFolder(Path, newFolder) {
	location.href = "files.php?datadir=<?=$datadir?>&dialogname=<?php echo $MY_NAME; ?>&createFolder=1&path="+Path+'&file='+newFolder;
}
...
if($d) {
	...
	<thead>
		<tr>
			<td width="2%" id="sortmefirst" onclick="st2.sort(5)">'.$MY_MESSAGES['type'].'</td>
			<td width="55%" title="CaseInsensitiveString" onclick="st1.sort(1);st2.sort(1)">'.$MY_MESSAGES['name'].'</td>
	...
	while (false !== ($entry = $d->read())) {
		if(substr($entry,0,1) != '.') {
			...
			if (is_dir($absolutePath)) {
				...
				$t_folders .= '<tr >
				<td width="2%"><img src="images/ext/folder_small.gif" width="16" height="16" border=0 alt="'.$entry.'" /></td>
				<td width="55%"><div style="width:200px; overflow:hidden;"><a href="files.php?datadir='.$datadir.'&dialogname='.$MY_NAME.'&path='.$relativePath.'" onClick="changeLoadingStatus(\'load\')">'.$entry.'</a></div></td>
				<td width="10%" align="right">'.$MY_MESSAGES['folder'].'</td>
				<td width="15%" align="right">'.parse_time($time).'</td>
				<td width="8%" align="center">';
				if ($MY_ALLOW_DELETE_FOLDER) {
					$t_folders .= '<a href="files.php?datadir='.$datadir.'&dialogname='.$MY_NAME.'&deleteFolder=1&path='.$MY_PATH.'&file='.$entry.'" onClick="return deleteFolder(\''.$entry.'\');"><img src="images/edit_trash.gif" width="15" height="15" border="0"></a>';
				} else { 
					$t_folders .= '&nbsp;';
				}
				$t_folders .= '</td>
				...
				$t_files .= '<tr>
				<td width="2%"><img src="'.$parsed_icon.'" width="16" height="16" border=0 alt="'.$entry.'" /></td>
				<td width="55%"><div style="width:200px; overflow:hidden;"><a href="javascript:;" onClick="javascript:fileSelected(\''.$MY_BASE_URL.$relativePath.'\',\''.$entry.'\',\''.$parsed_icon.'\',\''.$parsed_size.'\',\''.$parsed_time.'\');">'.$entry.'</div></td>
				<td width="10%" align="right">'.$parsed_size.'</td>
				<td width="15%" align="right">'.$parsed_time.'</td>
				<td width="8%" align="center">';
				if ($MY_ALLOW_DELETE_FILE) {
					$t_files .= '<a href="files.php?datadir='.$datadir.'&dialogname='.$MY_NAME.'&deleteFile=1&path='.$MY_PATH.'&file='.$entry.'" onClick="return deleteFile(\''.$entry.'\');"><img src="images/edit_trash.gif" width="15" height="15" border="0"></a>';
				} else { 
					$t_files .= '&nbsp;';
				}
				$t_files .= '</td>
