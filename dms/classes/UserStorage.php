...
class UserStorage {
	...
	function lookupUser($login) {
		...
		$sql = "SELECT id, login, password, title, firstname, surname, email, category 
				FROM users WHERE login = '".$login."'";
		/*		
		$sql = "SELECT id, login, password, title, firstname, surname, email, category 
				FROM users JOIN dms_admin ON users.id=dms_admin.users WHERE login = '".$login."'";
		*/
	...
	function createUserObject($row) {			
		...
		$user =  new User($id, $login, $password, $title, $fname, $lname, $email, $category);
		return $user;
	}
