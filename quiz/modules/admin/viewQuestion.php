...
$result=$quiz->SelectQuiz($quiz,$page);
@list($page,$totalpage)=$quiz->Page($quiz,$num,$page,25);			//return action page and totalpage
//====================Template=========================
					...
					while($rs = @$result->fetchRow(DB_FETCHMODE_ASSOC)){
						...
						if ($rs["question_type"]=="mltc"){
							...
							$template->assign_block_vars('block.list', array('QUES'=>$rs['question'],
																												...
																												'EDIT'=>"<a href=# onClick=editQuestion(".$rs['question_id'].",".$quiz->getModules().");><img src=\"../images/edit4.gif\" border=\"0\" align=\"middle\"></a>",
																												'REMOVE'=>"<a href=# onClick=remove_question(".$rs['question_id'].",".$quiz->getModules().");>".$strRemove."</a>",
																												'DELETE'=>"<a href=#  onClick=check_delete_question(".$rs['question_id'].",".$quiz->getModules().");><img src=\"../images/_trash_full-16.png\" border=\"0\" align=\"middle\"></a>",
																												));
				}
