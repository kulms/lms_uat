...
include('classes/main_functions.php');
require("header.php");
...
$SD = standard_deviation($score);
$Mean= mean($score);
$Median = median($score) ;
//$xx = modevalue($score);
$Mode="";
		$mod = array_count_values($score);
		 rsort($mod);
		//echo $mod[0];
		$modx = array_count_values($score);
		 while(list($key,$val) = each($modx)) { 
					if($val== $mod[0]) {  // echo "=$key==". $val; 
						$modearr[] = $key;
					}
			} 
	for($i=0;$i<count($modearr);$i++){
				if($i==count($modearr)-1) $Mode .= $modearr[$i];
				else $Mode .= $modearr[$i].",";
		} 
