...
$template->set_filenames(array('body' => 'main.html',
		...
		while($rs = @$result->fetchRow(DB_FETCHMODE_ASSOC)){
				...
				//**************GET  NAME of  UID*******************
			@list($to_id,$to_fname,$to_sname) =  $message->GetDetailPerson($message,$rs[msg_to_uid]);
			@list($from_id,$from_fname,$from_sname) =  $message->GetDetailPerson($message,$rs[msg_from_uid]);
				$template->assign_block_vars('list.topiclist', array(
																...
																'BG_C'=>$color[$i%2] ,
																'FROM'=>($toperson==1)?"$to_fname  $to_sname":"$from_fname $from_sname",
